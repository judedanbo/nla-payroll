...
const verificationStatus = ref<'present' | 'absent' | 'on_leave' | 'ghost'>('present')
    ...
    <div class="container mx-auto space-y-6 py-6">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div>
            <Heading>Staff Verification</Heading>
            <p class="text-muted-foreground text-sm">{{ session.session_name }}</p>
          </div>
        </div>
      </div>
        ...
        <Card class="lg:col-span-1">
          ...
          <CardContent class="space-y-4">
            ...
            <div class="space-y-2">
              ...
              <p v-if="selectedStation" class="text-muted-foreground text-xs">
                {{ selectedStation.city }} • {{ selectedStation.staff_count }} staff members
              </p>
            </div>
            ...
            <!-- Staff List -->
            <div v-if="selectedStationId" class="space-y-2">
              <Label>Unverified Staff ({{ unverifiedStaff.length }})</Label>
              <div class="max-h-96 space-y-1 overflow-y-auto rounded-md border p-2">
                ...
                >
                  ...
                  <div class="text-muted-foreground text-xs">{{ staff.staff_number }}</div>
                  <div class="text-muted-foreground text-xs">{{ staff.job_title }}</div>
                </button>
                <p v-if="unverifiedStaff.length === 0" class="text-muted-foreground py-4 text-center text-sm">
              ...
              <details v-if="verifiedStaff.length > 0" class="group">
                <summary class="text-muted-foreground flex cursor-pointer items-center gap-2 text-sm">
                ...
                <div class="mt-2 space-y-1 rounded-md border p-2">
                  <div
                    ...
                    :key="staff.id"
                    class="bg-muted/50 rounded-md p-2"
                  >
                    <div class="flex items-center justify-between">
                      <div>
                        <div class="text-sm font-medium">{{ staff.full_name }}</div>
                        <div class="text-muted-foreground text-xs">{{ staff.staff_number }}</div>
                      </div>
                      <UserCheck class="text-green-600 size-4" />
                    </div>
                  </div>
                </div>
              </details>
        ...
        <Card class="lg:col-span-2">
          ...
          <CardContent>
            <form @submit.prevent="handleSubmit" class="space-y-6">
              <!-- Selected Staff Info -->
              <div v-if="selectedStaff" class="bg-muted/50 rounded-lg border p-4">
                <div class="flex items-start justify-between">
                  <div>
                    <h4 class="font-semibold">{{ selectedStaff.full_name }}</h4>
                    <p class="text-muted-foreground text-sm">{{ selectedStaff.staff_number }}</p>
                    <p class="text-muted-foreground text-sm">
                      {{ selectedStaff.job_title }} • {{ selectedStaff.department }}
                    </p>
                  </div>
              ...
              <div v-else class="text-muted-foreground rounded-lg border border-dashed p-8 text-center">
                <Users class="text-muted-foreground/50 mx-auto mb-2 size-12" />
                <p class="text-sm">Select a staff member to begin verification</p>
              </div>
              ...
              <div v-if="selectedStaff" class="space-y-2">
                ...
                <Select v-model="verificationStatus">
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="ghost">Ghost Employee</SelectItem>
                  </SelectContent>
                </Select>
              </div>
